...
const NewProduct = () => {
  ...
  const newProductSubmitHandler = (e) => {
    ...
    formData.append("cuttedPrice", cuttedPrice || 0);
    formData.append("category", category || "");
    formData.append("stock", stock || 0);
    formData.append("warranty", warranty || 0);
    formData.append("brandname", brand || "Default Brand");

    // Add Cloudinary URLs if provided (format: "url1,url2,url3")
    if (imageUrls.trim()) {
    ...
    // DO NOT send logo or images - they're optional
    // Only add highlights - must have at least 1
    ...
    // Only add specifications - must have at least 2
    specs.forEach((s) => {
      if (s && s.title && s.description) {
        formData.append("specifications", JSON.stringify(s));
      }
    });

    console.log(
      "Submitting product with FormData keys:",
      ...
      >
        ...
        <div className="flex flex-col gap-2 m-2 sm:w-1/2">
          ...
          <TextField
            ...
            variant="outlined"
            size="small"
            placeholder="Paste Cloudinary URLs separated by commas&#10;Example: https://res.cloudinary.com/...url1, https://res.cloudinary.com/...url2"
            value={imageUrls}
            onChange={(e) => setImageUrls(e.target.value)}
            fullWidth
            helperText="Optional: Paste Cloudinary image URLs separated by commas"
          />
          ...
          <label className="rounded font-medium bg-gray-400 text-center cursor-pointer text-white p-2 shadow hover:shadow-lg my-2">
            <input
              ...
              className="hidden"
            />
            Choose Files (Alternative - File Upload)
          </label>
